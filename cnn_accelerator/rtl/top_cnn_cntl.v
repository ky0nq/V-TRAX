...
// =============================================================================
// top_cnn_cntl.v  -  제어부 묶음 파일
//
// 다른 그룹 파일(act_path.v / wgt_path.v / pe_core.v / out_path.v)과 같은
// 구성이다. 하위 모듈을 먼저 두고, 묶음 모듈 top_cnn_cntl 을 맨 뒤에 둔다.
//
//   cnn_cntl       레이어 · 타일 스케줄러
//   pe_cntl        타일 하나의 cycle 단위 제어기
//   top_cnn_cntl   위 둘을 묶은 블록도상의 제어부
// =============================================================================


// =============================================================================
// cnn_cntl : 레이어 · 타일 스케줄러  (명세 62행 / 07절 FSM)
//
//   C_IDLE -> C_START -> C_PARAM_LOAD -> C_SET -> C_W_LOAD -> C_TILE_CFG
//                          (추론당 1회)        ^         ^              |
//                                              |         |          C_PE_TILE
//                                              |         |              |
//                                              |         |          C_PE_WAIT
...
//                                                  C_DONE
//
// 이 블록이 직접 하는 일 (명세 63행)
//   - 레이어 descriptor / 출력 경로 설정 / 타일 설정 발행
//   - Weight Chunk 적재 명령 (첫 Chunk 는 C_W_LOAD, 후속은 C_PE_WAIT)
//   - Parameter RAM 읽기와 param_buf 쓰기 (독립 Init 모듈을 두지 않는다)
//   - 공유 RAM 소유권 전환 (i_ram_idle=1 에서만)
//
// -----------------------------------------------------------------------------
// Parameter 는 추론 시작에 한 번에 다 불러온다 (2026-09-20, 사용자 결정)
//
//   C_PARAM_LOAD 는 타일 루프 밖에 있고, 레코드 0 .. PARAM_TOTAL-1 을 순서대로
//   param_buf 로 옮긴 뒤 다시 오지 않는다. param_buf 가 1<<PARAM_AW = 128
//   레코드라 Candidate ID 32 의 43 레코드가 통째로 들어간다.
//
//   이전 구조는 타일마다 유효 col 의 레코드 3 개를 다시 읽었다. 목적지 주소가
//   원본 주소와 같아서 (o_param_wr_addr = 읽은 주소) param_buf 의 최종 내용은
//   양쪽이 똑같고, post_process 가 보는 것도 달라지지 않는다. 달라지는 것은
//     - Param RAM 읽기가 추론당 PARAM_TOTAL 번으로 끝난다. Candidate ID 32
//       기준 43 번. 이전에는 타일마다 유효 col 을 다시 읽어 9,597 번이었다
//       (tb_cnn_cntl_full 로 실측)
//     - o_ram_owner 가 추론당 한 번만 param 쪽으로 갔다가 돌아온다.
//       타일 루프 전체에서 소유권은 weight(0) 에 고정이다
//     - 타일 설정(o_tile_cfg_valid) 발행과 params_ready 확인은 C_TILE_CFG 로
//       분리했다. 이전에는 C_PARAM_LOAD 꼬리가 겸하던 일이다
//
// -----------------------------------------------------------------------------
// 타일 순회 순서 (2026-09-21, 명세 v1.2 와 팀 그림에 맞춤)
//
//   위치(pos_group)가 바깥, 출력 채널 그룹(oc_group)이 안쪽이다.
//
//       for pg :  for og :  타일 (patch_base = 3*pg,  out_ch_base = 3*og)
//
//   명세 out_path.i_patch_base : "채널 그룹 변경 시 유지, 다음 위치 타일에서 +3".
//     - act_patch_gen 은 4x4 캡처로 패치를 만들어 들고 있다가, 같은 pos_base 가
//       다시 오면 읽지 않고 같은 패치를 k=0 부터 다시 낸다. 그래서 같은 위치의
//       채널 그룹을 연달아 돌려야 입력을 한 번만 읽는다.
//     - pooling_unit 은 그룹별로 (max, 다음 q, window_id) 를 들고 있어서 그룹이
//       섞여 들어와도 된다. line buffer 는 없다.
//
//   o_pos_base 는 raster 위치가 아니라 "패치 순번" 이다. 출력 위치를 2x2 블록
//   안에서 Z 순서로 센 번호이고 (W=64 : patch 0~8 -> pos 0,1,64,65,2,3,66,67,4),
//   좌표로 바꾸는 일은 act_patch_gen 과 output_fifo 가 한다. 제어부는 3 씩 늘리기만
//   하면 된다. 위치 타일 수 ceil(out_pix/3) 와 마지막 row_mask 는 전과 같다.
//
//   대가 : 출력 채널 그룹이 2개 이상인 Conv 는 타일마다 그룹이 바뀌므로 weight 를
//   매번 다시 적재한다 (wload_skip 은 그룹이 하나뿐인 레이어에서만 걸린다).
//   Conv0 기준 27 Word x 2,732 타일. 대신 입력 읽기는 패치당 9 픽셀에서 4 픽셀로
//   줄고, 채널 그룹이 바뀔 때 다시 읽지 않으며, 계산 중에 다음 블록을 미리 읽는다.
// =============================================================================
module cnn_cntl #(
    ...
    // ---- pe_cntl : Chunk 재적재 --------------------------------------------
    //   K > 64 (fc1) 에서 pe_cntl 이 타일 중간에 다음 chunk 를 요청하는 핸드셰이크.
    //   명세 cnn_cntl 표 "pe_cntl" 구간에 같은 이름 6 행이 들어가 있다 (설명 / 타이밍 칸은 아직 비어 있다).
    input  wire                  i_chunk_req_valid,      // <- pe_cntl.o_chunk_req_valid
    output wire                  o_chunk_req_ready,      // -> pe_cntl.i_chunk_req_ready
    input  wire [`K_W-1:0]       i_chunk_k_base,         // <- pe_cntl.o_chunk_k_base
    input  wire [`CHUNK_W-1:0]   i_chunk_len,            // <- pe_cntl.o_chunk_len
    output wire                  o_chunk_done_valid,     // -> pe_cntl.i_chunk_done_valid
    input  wire                  i_chunk_done_ready,     // <- pe_cntl.o_chunk_done_ready
    ...
    always @* begin
        ...
        case (layer_idx) // layer setting
            3'd0: begin                                    // Conv0 : L0_* = 64x64x3 -> 64x64x6, K 27, out_pix 4096, w_base 0, param_base 0, ReLU, Pool
                ...
                ly_relu = L0_RELU; ly_pool_en = L0_POOL_EN; ly_is_fc = 1'b0;
            ...
            3'd1: begin                                    // Conv1 : L1_* = 32x32x6 -> 32x32x4, K 54, out_pix 1024, w_base 54, param_base 6, ReLU
                ...
                ly_relu = L1_RELU; ly_pool_en = L1_POOL_EN; ly_is_fc = 1'b0;
    ...
    wire [`CHUNK_W-1:0] first_len = (ly_k > WBUF_WORDS) ? WBUF_WORDS[`CHUNK_W-1:0]  
                                                        : ly_k[`CHUNK_W-1:0];  // 첫 chunk 길이 = min(K, WBUF_WORDS=64) -> 27 / 54 / 64 / 32
    ...
    assign o_chunk_req_ready  = (state == C_PE_WAIT) && (wsvc == W_IDLE);
    assign o_chunk_done_valid = (wsvc == W_REPLY);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (wsvc)
                W_IDLE: begin
                    if ((state == C_PE_WAIT) && i_chunk_req_valid) begin
                        svc_kbase     <= i_chunk_k_base;
                        svc_len       <= i_chunk_len;
                        svc_for_chunk <= 1'b1;
                        wsvc          <= W_ISSUE;
                    end
                ...
                W_WAIT:  if (i_wload_done) begin              // 1clk 펄스. 적재 중에는 항상 이 상태라 놓치지 않는다
                    if (svc_for_chunk) begin
                ...
                default: if (i_chunk_done_ready) begin   // W_REPLY
                    svc_for_chunk <= 1'b0;
                    wsvc          <= W_IDLE;
                end
            endcase
            ...
            case (state)
                ...
                C_W_LOAD: begin
                    ...
                    else if (!first_chunk_rdy && (wsvc == W_IDLE) && !svc_for_chunk) begin
...
// =============================================================================
// pe_cntl : 타일 하나의 cycle 단위 제어기  (명세 132행 / 07절 FSM)
//
//   P_IDLE -> P_CLEAR -> P_PREFILL -> P_FEED -+-> P_DRAIN -> P_WAIT_OUT -> P_IDLE
//                            ^                |
//                            +- P_CHUNK_WAIT <+
//
// weight_buf 는 64 Word 한 개뿐이라 K 가 길면 Chunk 로 나눠 돈다 (명세 528행).
// Chunk 경계에서 지켜야 하는 것
//   - array / skew / 누산기를 지우지 않고 그대로 정지시킨다 (o_step_en = 0)
//   - o_acc_clear, o_tile_clear 는 P_CLEAR 에서만 나간다 (명세 545행 "clear 금지")
//   - o_mac_last 는 "전체 K 의 마지막 MAC" 에만 뜬다. Chunk 의 마지막이 아니다.
//     (여기서 잘못 뜨면 미완성 누산값이 output_fifo 로 flush 된다)
//
// 진행 규칙 (명세 525행)
//   inject     = FEED && act_valid && weight_valid && result_space_ready
//   step_en    = inject || (DRAIN && result_space_ready)
//   feed_valid = inject
//   ingress 인 feeder_en 은 별도이며 PREFILL 에서도 1 이다.
//
// MAC 타이밍 (명세 527행)
//   t=0 이 PE(0,0)의 첫 MAC 일 때 r+c <= t < K+r+c,
//   mac_last = step_en && mac_valid && (t == K-1+r+c).
//   valid_pipe/last_pipe 5단이 대각선 d=r+c (0..4) 를 그대로 표현한다.
// =============================================================================
module pe_cntl #(
    ...
    // ---- cnn_cntl : 타일 중간 Chunk 재적재 -----------------------------------
    //   명세 pe_cntl 표 "cnn_cntl" 구간에 같은 이름 6 행이 있다. 그 표의 "연결 대상" 칸이
    //   pe_cntl 로 적혀 있는데 cnn_cntl 이 맞다 (설명 / 타이밍 칸도 아직 비어 있다).
    output wire                  o_chunk_req_valid,      // -> cnn_cntl.i_chunk_req_valid   P_CHUNK_WAIT 이고 직전 chunk 의 i_chunk_done 을 본 뒤 1
    input  wire                  i_chunk_req_ready,      // <- cnn_cntl.o_chunk_req_ready   (C_PE_WAIT 이고 적재 서비스가 한가할 때 1)
    output wire [`K_W-1:0]       o_chunk_k_base,         // -> cnn_cntl.i_chunk_k_base      다음 chunk 가 시작하는 k
    output wire [`CHUNK_W-1:0]   o_chunk_len,            // -> cnn_cntl.i_chunk_len         다음 chunk 길이 = min(남은 K, 64)
    input  wire                  i_chunk_done_valid,     // <- cnn_cntl.o_chunk_done_valid  다음 chunk 가 wgt_buf 에 다 들어왔다
    output wire                  o_chunk_done_ready,     // -> cnn_cntl.i_chunk_done_ready
    ...
    localparam [2:0] P_WAIT_OUT   = 3'd6;

    // WBUF_WORDS = 64 = wgt_buf 의 word 수. K 와 비교해야 해서 K_W 폭으로 맞춘다.
    localparam [`K_W-1:0] WBUF_LEN = WBUF_WORDS[`K_W-1:0];   // 13'd64. K > 64 면 chunk 로 나눠 돈다

    reg [2:0]          state;
    reg                tile_in_done_seen;

    reg [`K_W-1:0]     k_total;    // `define K_W    13 = K 1 ~ 4096. 이 타일 전체의 MAC 수 (Conv0 27 / Conv1 54 / fc1 4096 / fc2 32)
    reg [`K_W-1:0]     k_base;     // `define K_W    13. 지금 chunk 가 시작하는 k (fc1 : 0, 64, 128, … 4032)
    reg [`CHUNK_W-1:0] k_cnt;      // `define CHUNK_W 7 = 1 ~ 64. 이번 chunk 안에서 몇 번째 beat 인가 (0 … cur_len-1)
    reg [`CHUNK_W-1:0] cur_len;    // `define CHUNK_W 7 = 1 ~ 64. 지금 chunk 의 길이 = min(남은 K, 64)

    reg [`KEEP_W-1:0]  row_mask_q, col_mask_q; // `define KEEP_W 3 = lane 3개. 유효 출력 위치(PE 행) / 유효 출력 채널(PE 열). 꼬리 타일은 001, 011
    ...
    // ---- Chunk 계산 --------------------------------------------------------
    wire [`K_W-1:0]     k_remain = k_total - k_base;
    wire [`CHUNK_W-1:0] next_len = (k_remain > WBUF_LEN) ? WBUF_LEN[`CHUNK_W-1:0]   // 다음 chunk 길이 = min(남은 K, WBUF_LEN=64). fc1 은 항상 64
                                                         : k_remain[`CHUNK_W-1:0];

    wire [`K_W-1:0]     first_full = (i_step_total == {`K_W{1'b0}}) ? {{(`K_W-1){1'b0}},1'b1}
                                                                     : i_step_total;
    wire [`CHUNK_W-1:0] first_len  = (first_full > WBUF_LEN) ? WBUF_LEN[`CHUNK_W-1:0]   // 첫 chunk 길이 = min(K, WBUF_LEN=64)
                                                             : first_full[`CHUNK_W-1:0];
    wire [`K_W-1:0]     k_next = k_base + {{(`K_W-`CHUNK_W){1'b0}}, cur_len};

    wire inject = (state == P_FEED) && i_act_valid && i_weight_valid &&
                  i_result_space_ready;

    wire beat_chunk_last  = (k_cnt == (cur_len - 1'b1));
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            state             <= P_IDLE;
            k_total           <= {{(`K_W-1){1'b0}},1'b1};      // `define K_W    13 → 13'd1 : 0 이면 beat_chunk_last 계산(cur_len-1)이 깨지므로 최소 1
            k_base            <= {`K_W{1'b0}};                 // `define K_W    13 → 13'd0 : 첫 chunk 는 k = 0 부터
            k_cnt             <= {`CHUNK_W{1'b0}};             // `define CHUNK_W 7 →  7'd0 : chunk 안의 beat 번호
            cur_len           <= {{(`CHUNK_W-1){1'b0}},1'b1};  // `define CHUNK_W 7 →  7'd1 : 위와 같은 이유로 최소 1
            row_mask_q        <= 3'b000;
            col_mask_q        <= 3'b000;
            req_sent          <= 1'b0;
            ...
            // reader_done 은 1clk 펄스라 플래그로 기억한다 (명세 149행)
            if (i_chunk_done)
                reader_done_seen <= 1'b1;

            // tile_in_done 도 1clk 펄스다 (명세 164행 "pulse 를 seen 으로 기억").
            // 유효 lane 이 하나뿐인 꼬리 타일은 출력이 한 beat 라 아직 P_DRAIN
            // 인 동안 펄스가 지나가 버린다. P_WAIT_OUT 에서 레벨만 보면 그
            // 펄스를 놓치고 영원히 멈춘다 (o_wbuf_free 도 0 이라 cnn_cntl 의
            // C_W_LOAD 까지 같이 멈춘다).
            if (i_tile_in_done)
                tile_in_done_seen <= 1'b1;
            ...
            case (state)
                ...
                // 1clk. 적재된 Chunk 를 local addr 0 부터 읽도록 Reader 재시작.
                P_PREFILL: begin
                    reader_done_seen <= i_chunk_done;
                    state            <= P_FEED;
                end
                ...
                // step=0 으로 상태를 그대로 유지하고 재적재만 처리한다.
                P_CHUNK_WAIT: begin
                ...
                // Array 는 정지. 후처리 입력 수락 완료만 기다린다.
                // Pool 결과 생성이나 레이어 저장 완료는 여기서 기다리지 않는다.
                P_WAIT_OUT: begin
                    if (i_tile_in_done || tile_in_done_seen)
                        state <= P_IDLE;
                end
...
// =============================================================================
// top_cnn_cntl  (블록도의 top_cnn_cntl 묶음) : cnn_cntl + pe_cntl
//
// cnn_cntl 은 레이어 / 타일 스케줄러, pe_cntl 은 그 아래 cycle 단위 제어기다.
// 포트는 연결 대상 블록별로 묶어 놨다.
//
// -----------------------------------------------------------------------------
...
//
//       pe_cntl                                  cnn_cntl
//       o_chunk_req_valid ------------------>  i_chunk_req_valid
//       i_chunk_req_ready <------------------  o_chunk_req_ready
//       o_chunk_k_base (13) ---------------->  i_chunk_k_base
//       o_chunk_len    ( 7) ---------------->  i_chunk_len
//       i_chunk_done_valid <----------------   o_chunk_done_valid
//       o_chunk_done_ready ----------------->  i_chunk_done_ready
//
//     cnn_cntl 은 C_PE_WAIT 에서 이 요청을 받아 wgt_ld_unit 명령으로 바꾼다.
//     그 동안 타일/레이어 index 는 바뀌지 않는다 (명세 548행).
//
// [3] 공유 완료 신호
//
...
module top_cnn_cntl #(
    ...
    parameter [`DIM_W-1:0] L0_IN_W = 7'd64,  parameter [`DIM_W-1:0] L0_IN_H = 7'd64,
    parameter [`CH_W-1:0]  L0_IN_C = 6'd3,   parameter [`DIM_W-1:0] L0_OUT_W = 7'd64,
    parameter [`DIM_W-1:0] L0_OUT_H = 7'd64, parameter [`CH_W-1:0]  L0_OUT_C = 6'd6,
    ...
    parameter              L0_RELU = 1'b1,   parameter              L0_POOL_EN = 1'b1,

    parameter [`DIM_W-1:0] L1_IN_W = 7'd32,  parameter [`DIM_W-1:0] L1_IN_H = 7'd32,
    parameter [`CH_W-1:0]  L1_IN_C = 6'd6,   parameter [`DIM_W-1:0] L1_OUT_W = 7'd32,
    parameter [`DIM_W-1:0] L1_OUT_H = 7'd32, parameter [`CH_W-1:0]  L1_OUT_C = 6'd4,
    parameter [1:0]        L1_STRIDE = 2'd1, parameter              L1_PAD_EN = 1'b1,
    ...
    parameter              L1_RELU = 1'b1,   parameter              L1_POOL_EN = 1'b0,

    parameter [`DIM_W-1:0] L2_IN_W = 7'd32, parameter [`DIM_W-1:0] L2_IN_H = 7'd32,
    ...
    parameter [`DIM_W-1:0] L3_IN_W = 7'd1,  parameter [`DIM_W-1:0] L3_IN_H = 7'd1,
    ...
    // -------------------------------------------------------------------------
    // 포트 옆 주석 읽는 법  (배선은 cnn_accel_top.v 의 U_TOP_CNN_CNTL 기준)
    //   [C] = 안에서 cnn_cntl 이 내거나 받는다     [P] = pe_cntl     [C+P] = 둘 다
    //   ->  = 이 출력이 이어지는 곳   "묶음모듈.포트 (그 안의 실제 서브모듈.포트)"
    //   <-  = 이 입력을 주는 곳
    // -------------------------------------------------------------------------
    input  wire                  clk,
    input  wire                  rst_n,
    ...
    assign o_pe_col_mask      = o_col_mask;         // 명세대로 cnn_cntl 의 mask 가 weight 쪽으로 간다 (pe_cntl.o_col_mask 포트는 없앴다)

    // 팀 결정 (2026-09-21) 으로 바뀐 방식은 이 모듈의 포트에 그대로 나간다.
    // 이 저장소의 예전 데이터패스에 맞추는 변환은 cnn_accel_top 의 "방식 다리" 블록에 있다.
    ...
    // ===== [2] Chunk 재적재 =====
    wire               chunk_req_valid, chunk_req_ready;
    wire [`K_W-1:0]    chunk_k_base;
    wire [`CHUNK_W-1:0] chunk_len;
    wire               chunk_done_valid, chunk_done_ready;

    assign o_pe_cmd_ready_dbg = pe_cmd_ready;

    cnn_cntl #(
        .NUM_LAYERS(NUM_LAYERS), .WBUF_WORDS(WBUF_WORDS),
        .REGION_A_BASE(REGION_A_BASE), .REGION_B_BASE(REGION_B_BASE),
        .INPUT_ZP(INPUT_ZP),
        .L0_IN_W(L0_IN_W), .L0_IN_H(L0_IN_H), .L0_IN_C(L0_IN_C),
        .L0_OUT_W(L0_OUT_W), .L0_OUT_H(L0_OUT_H), .L0_OUT_C(L0_OUT_C),
        .L0_STRIDE(L0_STRIDE), .L0_PAD_EN(L0_PAD_EN),
        .L0_K(L0_K), .L0_OUT_PIX(L0_OUT_PIX),
        .L0_W_BASE(L0_W_BASE), .L0_PARAM_BASE(L0_PARAM_BASE),
        .L0_RELU(L0_RELU), .L0_POOL_EN(L0_POOL_EN),
        .L1_IN_W(L1_IN_W), .L1_IN_H(L1_IN_H), .L1_IN_C(L1_IN_C),
        .L1_OUT_W(L1_OUT_W), .L1_OUT_H(L1_OUT_H), .L1_OUT_C(L1_OUT_C),
        .L1_STRIDE(L1_STRIDE), .L1_PAD_EN(L1_PAD_EN),
        .L1_K(L1_K), .L1_OUT_PIX(L1_OUT_PIX),
        .L1_W_BASE(L1_W_BASE), .L1_PARAM_BASE(L1_PARAM_BASE),
        .L1_RELU(L1_RELU), .L1_POOL_EN(L1_POOL_EN),
        .L2_IN_W(L2_IN_W), .L2_IN_H(L2_IN_H), .L2_IN_C(L2_IN_C),
        .L2_K(L2_K), .L2_OUT_C(L2_OUT_C), .L2_W_BASE(L2_W_BASE),
        .L2_PARAM_BASE(L2_PARAM_BASE), .L2_RELU(L2_RELU),
        .L3_IN_W(L3_IN_W), .L3_IN_H(L3_IN_H), .L3_IN_C(L3_IN_C),
        .L3_K(L3_K), .L3_OUT_C(L3_OUT_C), .L3_W_BASE(L3_W_BASE),
        .L3_PARAM_BASE(L3_PARAM_BASE), .L3_RELU(L3_RELU),
